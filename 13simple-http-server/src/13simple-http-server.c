 ...
 *             : 返回 启动的文件描述符
 */
int server_start(uint16_t* pport);
...
int main(int argc, char* argv[]) {
  pthread_attr_t attr;
  uint16_t port = 0;
  int sfd = server_start(&port);

  printf("httpd running on port %u.\n", port);
  // 初始化线程属性
  pthread_attr_init(&attr);
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED); // @suppress("Symbol is not resolved")
  for (;;) {
    pthread_t tid;
    ...
    if (cfd < 0) {
      LOG_ERROR("accept sfd = %d is error!", sfd);
      break;
    }
    if (pthread_create(&tid, &attr, request_accept, &cfd) < 0) {
      LOG_ERROR("pthread_create run is error!");
    }
  }
  // 销毁吧, 一切都结束了
...
void response_file(int cfd, const char* path) {
  ...
  // 读取报文头,就是过滤
  while (readLine(cfd, buf, sizeof buf) > 0 && strcmp("\n", buf))
    ;
  // 这里开始处理 文件内容
...
int server_start(uint16_t* pport) {
  ...
  ERROR_CHECK(sfd = socket(PF_INET, SOCK_STREAM, 0));

  saddr.sin_port = !pport || !*pport ? 0 : htons(*pport);
  saddr.sin_addr.s_addr = INADDR_ANY;
  socklen_t socklen = sizeof saddr;
  // 绑定一下端口信息
  ERROR_CHECK(bind(sfd, (struct sockaddr* )&saddr, socklen));
  if (pport && !*pport) {
    ERROR_CHECK(getsockname(sfd, (struct sockaddr* )&saddr, &socklen));
    *pport = ntohs(saddr.sin_port);
  }
  // 开启监听任务
  ERROR_CHECK(listen(sfd, _INT_LIS));
  return sfd;
}
...
void *request_accept(void *arg) {
  ...
  int *p = arg;
  int cfd = *p;
  int readLineLen = readLine(cfd, buf, sizeof buf);
  if (readLineLen <= 0) { //请求错误
    response_501(cfd);
    close(cfd);
    return NULL;
  }
